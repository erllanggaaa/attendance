...
<title>ATTENDANCE APP - ADMIN [DATA JABATAN]</title>
...
<tbody>
    <?php
    include('include/koneksi.php');
        $jabatan = mysqli_query ($konek, "SELECT * FROM tbl_jabatan");
            if($jabatan == false){
                ...
                while ($hasil = mysqli_fetch_array ($jabatan)){                            
                            ?>
                                <tr>
                                    <td><?=$hasil['id_jabatan'];?></td>
                                    <td><?=$hasil['nama_jabatan'];?></td>
                                    <td><a href="#" class="open_modal" title="Ubah" id="<?=$hasil['id_jabatan'];?>"><i class="glyph-icon icon-edit" aria-hidden="true"></i></a> |
                                        <a href="Delete-Jabatan?id_jabatan=<?=$hasil['id_jabatan'];?>" title="Hapus" onclick="return confirm(\'Anda yakin akan menghapus <?=$hasil['nama_jabatan'];?>?\')"><span class="glyph-icon icon-trash" aria-hidden="true"></span></a>
                    </td>
                                </tr>
                                <?php
                        }
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title"><i class="glyph-icon icon-plus"> <strong>Tambah Data Jabatan</strong></i></h4>
                    </div>
                    <div class="modal-body">
                        <form action="Tambah-Jabatan" name="modal_popup" enctype="multipart/form-data" method="post">
        ...
        <div id="ModalUbahJabatan" class="modal fade" tabindex="-1" role="dialog"></div>
